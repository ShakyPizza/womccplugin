...
- Automatic refresh every 1 hour, if enabled
...
This repository is developed as a RuneLite Plugin Hub style plugin. RuneLite's
[Developer Guide](https://github.com/runelite/runelite/wiki/Developer-Guide)
describes the basic architecture used here: `WomClanPlugin` is the
`@PluginDescriptor` plugin class, `WomClanConfig` is the saved config interface,
and `WomClanPanel` is the Swing sidebar panel. Keep future `@Subscribe` event
handlers on the plugin class.
